        ...
        for i, det in enumerate(pred):  # i:image index  det:(num_nms_boxes, [xylsθ,conf,classid]) θ∈[0,179]
            ...
            if det is not None and len(det):
                ...
                for *rbox, conf, cls in reversed(det):  # 翻转list的排列结果,改为类别由小到大的排列
                    # rbox=[tensor(x),tensor(y),tensor(w),tensor(h),tensor(θ)] θ∈[0,179]
...
def detectserver(image, save_img=False):
    '''
    input: save_img_flag
    output(result):
    '''
    ...
    imgsz = 1024
    out, source, weights = \
        './output/', './input/img.jpg', './detect.pt'
    webcam = source.isnumeric() or source.startswith(('rtsp://', 'rtmp://', 'http://')) or source.endswith('.txt')
    ...
    for path, img, im0s, vid_cap in dataset:
        ...
        # pred : list[tensor(batch_size, num_conf_nms, [xylsθ,conf,classid])] θ∈[0,179]
        # pred = non_max_suppression(pred, opt.conf_thres, opt.iou_thres, classes=opt.classes, agnostic=opt.agnostic_nms)
        pred = rotate_non_max_suppression(pred, 0.4, 0.4, classes=0,
                                          agnostic=False, without_iouthres=False)
        t2 = time_synchronized()
        ...
        for i, det in enumerate(pred):  # i:image index  det:(num_nms_boxes, [xylsθ,conf,classid]) θ∈[0,179]
            ...
            if det is not None and len(det):
                ...
                for *rbox, conf, cls in reversed(det):  # 翻转list的排列结果,改为类别由小到大的排列
                    ...
                    #         f.write(('%g ' * 5 + '\n') % (cls, *xywh))  # label format
                    obj = {
                        'rbox':{'x':rbox[0].item(), 'y':rbox[1].item(), 'w':rbox[2].item(), 'h':rbox[3].item(), 'theta':rbox[4].item()},
